...
- **Area selection** — global hotkey `Ctrl+Alt+S` (or the tray icon / launcher button)
...
Trigger a capture with `Ctrl+Alt+S`, the tray menu, or by double-clicking the tray icon.
...
the renderer stretches each word of transparent text over its box
(`transform: scaleX`) so the browser's own text selection lines up with the pixels.
